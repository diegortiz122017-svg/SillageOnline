...
function rowToProduct(r) {
  return {
    ...
    notifySubscribers: !!r.notify_subscribers,
    notes:         r.notes,
    top:           r.top_notes,
    mid:           r.mid_notes,
    base:          r.base_notes,
...
function productToRow(p, now) {
  return [
    ...
    p.luxury       ? 1 : 0,
    p.family       || null,
    p.arrivedAt    ? new Date(p.arrivedAt) : null,
    p.notifySubscribers ? 1 : 0,
    p.notes        || null,
    p.top          || null,
    p.mid          || null,
    p.base         || null,
    ...
    await db.execute(`
      INSERT INTO products
        (id, brand, name, gender, price, decant_price, decant_price_5, decant_size_ml,
         ...
         colors, shape, photos, chords, chords_override, sort_order, home_favorite, image_zoom, active, created_at, updated_at)
      VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,1,?,?)
      ON DUPLICATE KEY UPDATE
        ...
        sort_order=VALUES(sort_order), home_favorite=VALUES(home_favorite),
        image_zoom=VALUES(image_zoom), updated_at=VALUES(updated_at)`,
...
async function updateProduct(id, p) {
  ...
  await db.execute(`
    UPDATE products SET
      ...
      top_intensity=?, mid_intensity=?, base_intensity=?,
      tagline=?, description=?, concentration=?, season=?, sillage=?, longevity=?,
      colors=?, shape=?, photos=?, chords=?, chords_override=?, sort_order=?, home_favorite=?, image_zoom=?, updated_at=?
    WHERE id=?`,
